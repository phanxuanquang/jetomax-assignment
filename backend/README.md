...
ASP.NET Core (.NET 10) backend for a realtime chat app: REST + SignalR API, Clean Architecture, backed by Supabase (Auth, Storage, Postgres) and Google Gemini for AI features.

For the full design, requirements, and data model, see [../docs/](../docs/) — start with [backend-system-design-and-architecture.md](../docs/backend-system-design-and-architecture.md).
...
Dependency direction is `Api → Infrastructure → Application → Domain`. See [architecture §4](../docs/backend-system-design-and-architecture.md#4-codebase-architecture) for the full rationale.
...
Full first-time setup (Supabase, Google OAuth, environment variables) is in [../docs/prerequisite-setups.md](../docs/prerequisite-setups.md) — follow that end-to-end on a fresh machine. The quick version below assumes Supabase is already set up.
...
Never commit real keys — use [user-secrets](https://learn.microsoft.com/aspnet/core/security/app-secrets) in development:
...
1. Clone the repo.
2. Install the [.NET SDK 10.x](https://dotnet.microsoft.com/download) matching `global.json`/the `.csproj` target framework.
3. `dotnet restore` from `backend/` — NuGet resolves everything, no other package manager involved.
