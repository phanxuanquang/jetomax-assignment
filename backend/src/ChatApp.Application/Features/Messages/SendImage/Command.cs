...
/// <summary>
/// Sends an image message into a conversation (F-5). The image itself was already uploaded
/// client-side directly to Storage; the backend only ever handles its URL. Blocked when the
/// conversation is read-only, unless the caller is its owner.
/// </summary>
